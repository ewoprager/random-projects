 ...
 *
 * @param newA       New Audio to add
 *
 */
static void addMusic(Audio * root, Audio * newA);
 ...
 *
 */
static void addAudio(Audio * root, Audio * newA);
...
{
    Audio * newA = (Audio*)calloc(1, sizeof(Audio));
    ...
    if(SDL_LoadWAV(filename, &(newA->audio), &(newA->bufferTrue), &(newA->lengthTrue)) == NULL)
    {
    ...
    newA->buffer = newA->bufferTrue;
    newA->length = newA->lengthTrue;
    (newA->audio).callback = NULL;
    (newA->audio).userdata = NULL;
...
static inline void playAudio(const char * filename, Audio * audio, uint8_t loop, int volume)
{
    ...
    if(filename != NULL)
    {
        ...
        newA = createAudio(filename, loop, volume);
    }
    else if(audio != NULL)
    {
        newA = (Audio*)malloc(sizeof(Audio));
        ...
        memcpy(newA, audio, sizeof(Audio));
    ...
    if(loop == 1)
    {
        addMusic((Audio *) (gDevice->want).userdata, newA);
    }
    else
    {
        addAudio((Audio *) (gDevice->want).userdata, newA);
    }
